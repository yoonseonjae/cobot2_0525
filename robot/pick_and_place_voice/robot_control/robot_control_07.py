...
class RobotController(Node):
        ...
        if get_keyword_future.result().success:
            ...
            for target in target_list:
                if target in target_positions:
                    ...
                    # RESET_HOME으로 홈 복귀가 완료되었다면 현 사이클 종료
    ...
    def wait_and_drop(self, target):
        ...
        while rclpy.ok():
            # RESET_HOME 이벤트가 들어오면 외력 대기 중단 + 복귀 완료까지 대기
            if robot_phase["reset_home_event"].is_set():
                ...
                while robot_phase["reset_home_event"].is_set() and rclpy.ok():
                    time.sleep(0.2)
                return
            # 안전정지/비상정지면 외력 체크 자체를 건너뜀
            if robot_phase.get("paused"):
                time.sleep(0.2)
                continue
            ret_x = check_force_condition(DR_AXIS_X, min=4, max=50, ref=DR_BASE)
            ret_y = check_force_condition(DR_AXIS_Y, min=4, max=50, ref=DR_BASE)
...
class SafetyCommandNode(Node):
    """/dsr01/safety_cmd 구독 전용 노드. 별도 executor에서 spin."""
    def __init__(self):
        super().__init__('robot_safety_cmd_listener')
        self.sub = self.create_subscription(
            String, f'/{ROBOT_ID}/safety_cmd', self._on_cmd, 10)
        self.get_logger().info(f'[SafetyCmdListener] /{ROBOT_ID}/safety_cmd 구독 시작')
...
def main(args=None):
    ...
    # reset_home_event 감시 → 메인 노드 컨텍스트에서 motion API 사용
    def _reset_watcher():
        while rclpy.ok():
